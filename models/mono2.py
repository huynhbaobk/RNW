...
class Mono2Model(LightningModule):
    ...
    def training_step(self, batch_data, batch_idx):
        ...
        if self.global_step%300 == 0:
            ...
            for frame_id in self.opt.frame_ids:
                # color = batch_data[("color", frame_id, 0)]
                color = batch_data[("color_equ", frame_id, 0)] if self.opt.use_equ else batch_data[("color_aug", frame_id, 0)]
                self.logger.experiment.add_images(f"input_color_frame_{frame_id}", normalize_image(color), self.global_step)

            # log output images
            for frame_id in self.opt.frame_ids[1:]:
                color_pred = outputs[("color", frame_id, 0)]
                self.logger.experiment.add_images(f"output_color_frame_{frame_id}", normalize_image(color_pred), self.global_step)

            # log depth
            disp = outputs[("disp", 0, 0)]
            self.logger.experiment.add_images(f"output_dis_0", normalize_image(disp), self.global_step)
    ...
    def configure_optimizers(self):
        ...
        sch = MultiStepLR(optim, milestones=[15], gamma=0.1)

        return [optim], [sch]

    def get_color_input(self, inputs, frame_id, scale):
        return inputs[("color_equ", frame_id, scale)] if self.opt.use_equ else inputs[("color_aug", frame_id, scale)]
